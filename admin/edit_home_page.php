	...
	<?php 
		...
		if(isset($_POST['product_sbt'])){
          ...
          if(empty($_FILES['image']['name'])){
            $ads_select="select * from home_product where id='".$id."'";
		    $ads_query=mysqli_query($db,$ads_select);
		    $ads_row=mysqli_fetch_assoc($ads_query);
		    $imagename=$ads_row['image'];
          }
		  $img_update="update home_product set title='".$title."',description='".$description."',regular_price='".$r_price."',current_price='".$c_price."',image='".$imagename."' where id='".$id."'";
		...
		$ads_current_price=$ads_row['current_price'];
		$ads_regular_price=$ads_row['regular_price'];
	...
	<div class="add_product_main">
		<form method="post" enctype="multipart/form-data">
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input type="number" class="form-control" name="regular_price" value="<?php echo $ads_regular_price?>"/>
				</div>
			</div>
			<div class="form-group row">
				<label class="col-sm-2 col-form-label">Current Price</label>
				<div class="col-sm-10">
					<input type="number" class="form-control" name="current_price" value="<?php echo $ads_current_price?>"/>
				</div>
			</div>
			<div class="form-group row">
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<button class="btn waves-effect waves-light btn-success btn-skew" name="product_sbt" >Update</button>
				</div>
			</div>
	</form>
	</div>
